...
- KMM users can deploy a DRA kubelet plugin alongside a kernel module by specifying `spec.dra` on a Module resource.
...
- **FR-1:** The Module CRD must include a `spec.dra` field that accepts a DRA plugin configuration: container spec, init container, service account, volumes, `driverName` (required, DNS subdomain), and `deviceClasses` (optional list of DeviceClass definitions).
...
- **FR-5:** The admission webhook must reject creation of a Module with `spec.dra` when the cluster Kubernetes version is below 1.35.
...
- **NFR-2:** DRA features require Kubernetes >= 1.35. The webhook enforces this at admission time.
...
- [ ] The webhook rejects a Module with `spec.dra` on a cluster running Kubernetes < 1.35.
...
- The Kubernetes DRA API (`k8s.io/api/resource/v1`) is stable and available on clusters running version 1.35+.
...
- Kubernetes >= 1.35 with DRA support enabled.
